...
function normalizeConfig(value = {}) {
  ...
  return {
    manualValues: source.manualVotes || source.manual_votes || {},
    sections: normalizeSections(source.sections),
  };
}

async function loadAdminConfig() {
  if (!supabase) return normalizeConfig();
  const { data, error } = await supabase.from('portfolio_settings').select('value').eq('key', SETTINGS_KEY).maybeSingle();
  if (error) throw error;
  return normalizeConfig(data?.value);
}

function applyManualValues(baseRows, config) {
  const map = new Map((baseRows || []).map((row) => [row.option_id, numberValue(row.votes)]));
  for (const option of OPTIONS) {
    const extra = numberValue(config.manualValues?.[option.id]);
    if (extra !== 0) map.set(option.id, Math.max(0, numberValue(map.get(option.id)) + extra));
  }
  const rows = OPTIONS.map((option) => ({ ...option, votes: numberValue(map.get(option.id)) }));
  rows.sort((a, b) => b.votes - a.votes || a.title.localeCompare(b.title));
  const total = rows.reduce((sum, row) => sum + row.votes, 0);
  return { rows: rows.map((row) => ({ ...row, percent: total > 0 ? Math.round((row.votes / total) * 100) : 0 })), total };
}

function patchResults(rows, total) {
  const leader = rows[0];
  ...
  if (topThree) {
    topThree.innerHTML = '<h3>Top 3</h3>' + rows.slice(0, 3).map((row, index) => `
      <div class="rankCard"><span class="rank">#${index + 1}</span><div class="rankMain"><strong>${safe(row.title)}</strong><small>${safe(row.subtitle)}</small><div class="bar"><span style="width:${row.percent}%"></span></div></div><b>${row.votes} votes · ${row.percent}%</b></div>
    `).join('');
  }

  const distribution = document.querySelector('.distribution');
  if (distribution) {
    distribution.innerHTML = `<h3>All options</h3><p>${total} total votes</p>` + rows.map((row) => `<div class="miniRow"><span>${safe(row.title)}</span><b>${row.votes} · ${row.percent}%</b></div>`).join('');
  }
}

function patchSections(sections) {
  const panel = document.querySelector('.developmentPanel');
  if (!panel) return;
  panel.innerHTML = `<div class="developmentEyebrow">Model status</div><div class="developmentSections">${normalizeSections(sections).map((section) => `<div class="developmentSection"><div class="developmentSectionTitle">${safe(section.title)}</div><div class="developmentList">${section.items.map((item) => `<div class="developmentItem"><strong>${safe(item)}</strong></div>`).join('')}</div></div>`).join('')}</div><div class="developmentAuthor">CYBERPOP3D</div>`;
}

async function refreshOverlay() {
  if (!isResultsPage || !supabase) return;
  try {
    const [{ data: baseRows }, config] = await Promise.all([
      supabase.from('wishlist_vote_counts').select('option_id, votes'),
      loadAdminConfig(),
    ]);
    const merged = applyManualValues(baseRows || [], config);
    patchResults(merged.rows, merged.total);
    patchSections(config.sections);
  } catch (error) {
    console.warn('Live env admin overlay failed:', error);
  }
}

if (isResultsPage) {
  window.setTimeout(refreshOverlay, 800);
  window.setInterval(refreshOverlay, 20000);
}
